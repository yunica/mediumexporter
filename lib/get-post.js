...
module.exports = async function(mediumURL, params = {}) {
  ...
  if (s.virtuals.previewImage) {
    story.featuredImage = utils.image2file(s.virtuals.previewImage.imageId)
  }
  ...
  return Promise.all(promises)
    .then(async results => {
      ...
      if (options.frontmatter) {
        ...
        outputText += `lead: ""\n`
        if (story.subtitle) {
          outputText += `teaser: "${story.subtitle}"\n`
        } else {
          outputText += `teaser: ""\n`
        }
        outputText += `date: ${story.date}\n`

        // if (story.images.length > 0 && options.imagesHeader) {
        //   outputText += 'images:\n'
        ...
        if (story.featuredImage) {
          outputText += `    url: ./images/${story.featuredImage}\n`
        } else {
          outputText += `    url: \n`
        }
      ...
      if (output) {
        if (!!images.length && !options.jekyll) {
          outputPath = `${path.join(output, story.slug)}/${story.slug}.md`
        }
