...
function image2file(image, id_obj = {}) {
  // console.log(image, sanitize(image))
  let newname = sanitize(image.split('/')[image.split('/').length - 1])
  // check extension
  const extension = String(newname).slice(
    ((String(newname).lastIndexOf('.') - 1) >>> 0) + 2
  )
  if (extension === '') {
    newname = `${newname}.png`
  }
  if (id_obj && id_obj.id) {
    newname = `${id_obj.id}__${newname}`
  }
  return newname
}
...
function validateArrayMarkup(array_markups) {
  let isdelete = false
  return array_markups.map((m, k) => {
    if (m) {
      m = m.trim()
      if (isdelete) {
        isdelete = false
      ...
      const open_link = m.includes('[')
      const close_link = m.includes(']')
      const bold = m.includes('**')
      ...
      if (bold && array_markups.length >= k + 1 && array_markups[k + 1]) {
        const nex_mark = array_markups[k + 1].trim()
        if (nex_mark.includes('[')) {
          isdelete = true
          return ` [**`
        }
        if (nex_mark.includes(']')) {
      ...
      if (open_link && array_markups.length >= k + 1 && array_markups[k + 1]) {
        const nex_mark = array_markups[k + 1].trim()
        if (nex_mark.includes('**')) {
          isdelete = true
          return ` [**${m.replace('[', '')}`
      ...
      if (close_link && array_markups.length >= k + 1 && array_markups[k + 1]) {
        const nex_mark = array_markups[k + 1].trim()
        if (nex_mark.includes('**')) {
          isdelete = true
          return `**]${m.replace(']', '')} `
...
async function downloadImages(images, options = {}) {
  ...
  for (const image of images) {
    let file = image2file(image, options)
    ...
    setTimeout(() => {
      console.warn('img download => ', file)
    }, 500)
  }
...
function processSection(s, slug, images, options = {}) {
  ...
  if (s.backgroundImage) {
    ...
    const imgsrc = `${MEDIUM_IMG_CDN + Math.max(imgwidth * 2, 2000)}/${
      s.backgroundImage.id
    }`
    images.push(imgsrc)
    section = `\n![](${s.backgroundImage.id})`
    if (options.jekyll) {
      section = `\n![](/assets/media/${options.imageFolder +
        s.backgroundImage.id})`
    }
  }
  return section
...
async function getGitHubEmbed(iframesrc, options = {}) {
  ...
  if (json.payload.value.gist) {
    ...
    const gistJson = await r2.get(scriptsrc).json
    let mdSoureCode = ''
    for (const key in gistJson.files) {
      if (gistJson.files.hasOwnProperty(key)) {
        ...
        const gistCode = await r2.get(file.raw_url).text
...
async function processParagraph(p, slug, images, options = {}) {
  p.markups.sort(function(first, second) {
    return first.type < second.type
  })
  // const markups_array = createMarkupsArray(p.markups)
  const markups_array = validateArrayMarkup(createMarkupsArray(p.markups))
  ...
  // clear space
  const maches = [...p.text.matchAll(/(\*\*)/g)]
  maches.forEach((i, k) => {
    // inicio
    const index = i.index
    if (k % 2 == 0 && p.text[index + 2] === ' ') {
      p.text = replaceChar(p.text, '*', index + 2)
      p.text = replaceChar(p.text, ' ', index)
    } else if (k % 2 == 1 && p.text[index - 1] === ' ') {
      // cierre
      p.text = replaceChar(p.text, '*', index - 1)
      p.text = replaceChar(p.text, ' ', index + 1)
    }
  })
  ...
  switch (p.type) {
    ...
    case 4: // image & caption
      ...
      images.push(imgsrc)
      let text = `\n![${p.text}](./media/${image2file(p.metadata.id, options)})`
      if (options.jekyll) {
        text = `\n![${p.text}](/assets/media/${slug}/${image2file(
          p.metadata.id,
          options
        )})`
      }
